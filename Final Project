...
html"""
<p style="font-size:20px;">Team ChemFusion: Jasmine Deguzman, Alina Chisti, Nhi Nguyen, Juliana Chen</br>
...
The steps in our bioprocess involved finding the input stream value of sucrose, formulating the number of cell-free bioreactor chips we would be pairing in series to maximize the output of 3G, determining the number of stages needed to isolate 3G to 95% purity using a magic separator, and analyzing the financial feasibility of our process. 

Our bioprocesses will be operating under the assumptions that all reactors and streams are ideal, isothermal, and run at atmospheric pressure. Therefore, universal cooling or heating jackets are necessary in order for the reaction to run at the optimal temperature of 25℃. Additionally, the magical separation units used to separate each of the products are 75% effective (in the top branches) by mass, isothermal, and do not require power to run them. Last but not least, the disposal of unused byproducts is free of charge and has no effect on the environment.

To optimize our inputs, outputs, and other resources used, we found that feeding 14 mmol/hr of sucrose and glycerol at V_max_ with 17 chips in series gives us the best results. Using this process, we were able to maximize all of our outputs and consume all of the glycerol at V_max_, as well as make use of the reaction intermediate alpha-d-glucose which acted in place of sucrose when sucrose was depleted. Once we used our coded program in Pluto notebook to calculate the output values for all 3G and our 5 byproducts (carbon dioxide, acetate, water, methylglyoxal, and isoprene), we used the Tops mass fraction vs stage index graph and table provided to determine the number of separation stages we needed to produce 1.0 g/hr of 3G of 95% purity.  For the remaining outputs, we again used the Tops mass fraction vs stage index graph and table to determine the number of separation stages we needed to produce each byproduct with a profitable purity percentage. The purity percentage we determined for each of the byproducts was based on the standard marketable purity percentages on Sigma-Aldrich. We recognized that through selling the byproducts and maximizing their purity, we would be able to increase the revenue from our bioprocess to balance out the costs of our utility and resources. 

To evaluate the financial performance and economic feasibility of our bioprocess, we first calculated the fixed costs to set up our process, which includes hardwares and equipment, as well as the variable costs of reactants for a year.  This was then factored into our earnings made by selling 3G and other byproducts to get our yearly profit. We then compared it to alternative investments of 1% per year and 10% per year using a net present value approach. 

Though h2o is a byproduct of the series of reactions, our team did not attempt to separate/purify h2o because of safety reasons. As the magical separation units are only 75% efficient at every level, the best our team can do is approach the 100% purity level, but never reach it. This brings concerns to the fact that the 99% pure h2o also contains other harmful products that could bring about health concerns for the environment and consumers. Consequently, a regulated disposal system for the stream of h2o is necessary, but, in this ideal situation, we are making the assumption that such costs do not affect the profits of our project.
...
begin
	...
	mol_flow_values_feed_1 = [
		14 	; # sucrose mmol/hr (or maybe 6.1?)
	]
...
md"""
#### Financial Analysis with Net Present Value (NPV)

The net present value (NPV) of our project will be used to evaluate the relative value of our project turning sucrose into 3G. Additionally, the NPV will be used to rank-order whether our project is worthy of investment compared to an alternative investment yielding a minimum of 1% per year and a maximum of 10% per year (constant). NPV is the net cash flow for each time period discounted back to the present value:
...
After inputting a 14 mmol/hr stream of sucrose along with a 2.466 mmol/hr stream of glycerol into a two channel syringe pump that connects to a series of 17 chips, we produced an output of 3.10 g/hr of 3G, 1.06 g/hr of co2, 1.22 g/hr of acetate, 2.02 g/hr of methylglyoxal, 0.26 g/hr of isoprene, and 0.894 g/hr of h2o. However, after these products underwent the magic separator, we produced 1.0 g/hr of 3G at 95% purity, 0.11 g/hr of co2 at 99.8% purity, 0.22 g/hr of acetate at 99% purity,  3.2 g/hr of methylglyoxal above 40% purity, and 0.026 g/hr of isoprene at 99% purity. 
...
N = 17;  # number of chips in the series
...
The last two columns show the mass flow rate and mass fraction for component $i$ in the exit from chip $N$, which in this case is equal to 17.
"""
...
# ╔═╡ 889721ac-0b04-4aea-a6aa-4a5dd937b6a4
md"""
###### Separation of Main Product - 3G
"""

# ╔═╡ 28d5ecb6-3530-46f5-a112-72d2b6b4fd2a
# how many levels are we going to have in the separation tree?
number_of_levels2 = 5;

# ╔═╡ db96bfff-5d8d-491e-96e6-ac60f4a90a64
# However: the desired product has the opposite => correct for my compound of interest -> this is compound i = ⋆
idx_target_compound2 = find_compound_index(MODEL,:compound_name=>"propane-1,3-diol");

# ╔═╡ 4d7895e9-f439-4f99-a592-abde867d7e36
begin
	...
	for level = 2:number_of_levels2
		...
		m_dot_top = mass_dot_output.*(u2.^(level-1))
		m_dot_bottom = mass_dot_output.*(d2.^(level-1))

		# update my storage array -
		for species_index = 1:ℳ
			species_mass_flow_array_top2[species_index,level] = m_dot_top[species_index]
			species_mass_flow_array_bottom2[species_index,level] = m_dot_bottom[species_index]
		end
	end
...
# ╔═╡ a125b4fd-b52f-478a-a8a5-2835845cc963
begin
...
# ╔═╡ b0dc38ef-0d37-4665-8544-386c87a2409d
#space

# ╔═╡ 357ae453-1883-4644-a3b0-c812fe2e1061
md"""
###### Separation of Byproduct - co2
"""
...
# However: the desired product has the opposite => correct for my compound of interest -> this is compound i = ⋆
idx_target_compound = find_compound_index(MODEL,:compound_name=>"co2");
...
begin
	...
	plot(stages,species_mass_fraction_array_top[idx_target_compound,:], linetype=:steppre,lw=2,legend=:bottomright, 
		label="Mass fraction i = co2 Tops")
	xlabel!("Stage index l",fontsize=18)
	ylabel!("Tops mass fraction ωᵢ (dimensionless)",fontsize=18)

	# make a 0.998 line target line2 -
	target_line = 0.998*ones(number_of_levels)
	plot!(stages, target_line, color="red", lw=2,linestyle=:dash, label="Target 99.8% purity")
end
...
# ╔═╡ 44263264-3bf5-44e6-80ca-b144e38aa09e
#space

# ╔═╡ 4f85835d-d490-4731-947e-8a972bf5cafb
md"""
###### Separation of Byproduct - acetate
"""

# ╔═╡ bb676bf5-e639-4549-9c24-56d2b0ffc4d8
# how many levels are we going to have in the separation tree?
number_of_levels3 = 7;

# ╔═╡ 402745e7-56cc-4e1e-8e09-4858af157850
# However: the desired product has the opposite => correct for my compound of interest -> this is compound i = ⋆
idx_target_compound3 = find_compound_index(MODEL,:compound_name=>"acetate");

# ╔═╡ 6976e6e7-fe90-4ad4-8e33-fd89b367957f
begin
	...
	# correct defaults -
	u3[idx_target_compound3] = θ3
	d3[idx_target_compound3] = 1 - θ3
	...
	for species_index = 1:ℳ
		value = mass_dot_output[species_index]
		species_mass_flow_array_top3[species_index,1] = value
		species_mass_flow_array_bottom3[species_index,1] = value
	end
	
	for level = 2:number_of_levels3

		# compute the mass flows coming out of the top -
		m_dot_top = mass_dot_output.*(u3.^(level-1))
		m_dot_bottom = mass_dot_output.*(d3.^(level-1))

		# update my storage array -
		for species_index = 1:ℳ
			species_mass_flow_array_top3[species_index,level] = m_dot_top[species_index]
			species_mass_flow_array_bottom3[species_index,level] = m_dot_bottom[species_index]
		end
	end
...
# ╔═╡ 4958a9ba-a592-4b9a-9232-71f88d21ab32
begin
	...
	plot(stages3,species_mass_fraction_array_top3[idx_target_compound3,:], linetype=:steppre,lw=2,legend=:bottomright, 
		label="Mass fraction i = acetate Tops")
	xlabel!("Stage index l",fontsize=18)
	ylabel!("Tops mass fraction ωᵢ (dimensionless)",fontsize=18)
	
	# make a 0.99 line target line3 -
	target_line3 = 0.99*ones(number_of_levels3)
	plot!(stages3, target_line3, color="red", lw=2,linestyle=:dash, label="Target 99% purity")

end

# ╔═╡ b8ee1761-70f3-4328-9f0c-339858212c46
with_terminal() do

	# initialize some space -
	state_table = Array{Any,2}(undef, number_of_levels3, 3)
	for level_index = 1:number_of_levels3
		state_table[level_index,1] = level_index
		state_table[level_index,2] = species_mass_fraction_array_top3[idx_target_compound3, level_index]
		state_table[level_index,3] = total_mdot_top_array3[level_index]
	end
...
# ╔═╡ 2c2ed43f-7b87-47dd-8168-8f30964e21bc
#space

# ╔═╡ 7068c22c-2ca0-46e7-95de-bb8f9cdb25f7
md"""
###### Separation of Byproduct - methylglyoxal
"""

# ╔═╡ 9af01770-9925-453e-8d7c-0811b535b69f
# how many levels are we going to have in the separation tree?
number_of_levels4 = 2;

# ╔═╡ fcbfdbcb-5a5e-44b6-8880-697ea995c941
# However: the desired product has the opposite => correct for my compound of interest -> this is compound i = ⋆
idx_target_compound4 = find_compound_index(MODEL,:compound_name=>"methylglyoxal");

# ╔═╡ 1879923f-6fe5-4e99-8c59-87632607d2f5
begin
	...
	# correct defaults -
	u4[idx_target_compound4] = θ4
	d4[idx_target_compound4] = 1 - θ4
	...
	for species_index = 1:ℳ
		value = mass_dot_output[species_index]
		species_mass_flow_array_top4[species_index,1] = value
		species_mass_flow_array_bottom4[species_index,1] = value
	end
	
	for level = 2:number_of_levels4

		# compute the mass flows coming out of the top -
		m_dot_top = mass_dot_output.*(u4.^(level-1))
		m_dot_bottom = mass_dot_output.*(d4.^(level-1))

		# update my storage array -
		for species_index = 1:ℳ
			species_mass_flow_array_top4[species_index,level] = m_dot_top[species_index]
			species_mass_flow_array_bottom4[species_index,level] = m_dot_bottom[species_index]
		end
	end
...
# ╔═╡ c2934c46-789d-46cd-8a1a-800f3828ec8f
begin
	...
	plot(stages4,species_mass_fraction_array_top4[idx_target_compound4,:], linetype=:steppre,lw=2,legend=:bottomright, 
		label="Mass fraction i = methylglyoxal Tops")
	xlabel!("Stage index l",fontsize=18)
	ylabel!("Tops mass fraction ωᵢ (dimensionless)",fontsize=18)
	
	# make a 0.40 line target line4 -
	target_line4 = 0.40*ones(number_of_levels4)
	plot!(stages4, target_line4, color="red", lw=2,linestyle=:dash, label="Target 40% purity")

end

# ╔═╡ a27938af-e2bf-4b83-a1c4-8fea3d8221da
with_terminal() do

	# initialize some space -
	state_table = Array{Any,2}(undef, number_of_levels4, 3)
	for level_index = 1:number_of_levels4
		state_table[level_index,1] = level_index
		state_table[level_index,2] = species_mass_fraction_array_top4[idx_target_compound4, level_index]
		state_table[level_index,3] = total_mdot_top_array4[level_index]
	end
...
# ╔═╡ c66d0399-fc75-4711-a6e4-b8337a613494
#space

# ╔═╡ 72713e20-1998-4aa4-9b2c-6536868f5f84
md"""
###### Separation of Byproduct - isoprene
"""

# ╔═╡ 4a3ee7ca-f428-4855-abed-207543f5ab6e
# how many levels are we going to have in the separation tree?
number_of_levels5 = 9;

# ╔═╡ 777ced27-bb92-4dc4-91c0-910e7c421d73
# However: the desired product has the opposite => correct for my compound of interest -> this is compound i = ⋆
idx_target_compound5 = find_compound_index(MODEL,:compound_name=>"isoprene");

# ╔═╡ fbcd5079-3dcf-4ade-8baa-b812dd416c70
begin
	...
	# correct defaults -
	u5[idx_target_compound5] = θ5
	d5[idx_target_compound5] = 1 - θ5

	# let's compute the composition of the *always up* stream -
	
	# initialize some storage -
	species_mass_flow_array_top5 = zeros(ℳ,number_of_levels5)
	species_mass_flow_array_bottom5 = zeros(ℳ,number_of_levels5)

	for species_index = 1:ℳ
		value = mass_dot_output[species_index]
		species_mass_flow_array_top5[species_index,1] = value
		species_mass_flow_array_bottom5[species_index,1] = value
	end
	
	for level = 2:number_of_levels5

		# compute the mass flows coming out of the top -
		m_dot_top = mass_dot_output.*(u5.^(level-1))
		m_dot_bottom = mass_dot_output.*(d5.^(level-1))

		# update my storage array -
		for species_index = 1:ℳ
			species_mass_flow_array_top5[species_index,level] = m_dot_top[species_index]
			species_mass_flow_array_bottom5[species_index,level] = m_dot_bottom[species_index]
		end
	end
	
	# what is the mass fraction in the top stream -
	species_mass_fraction_array_top5 = zeros(ℳ,number_of_levels5)
	species_mass_fraction_array_bottom5 = zeros(ℳ,number_of_levels5)

	# array to hold the *total* mass flow rate -
	total_mdot_top_array5 = zeros(number_of_levels5)
	total_mdot_bottom_array5 = zeros(number_of_levels5)
	
	# this is a dumb way to do this ... you're better than that JV come on ...
	T_top5 = sum(species_mass_flow_array_top5,dims=1)
	T_bottom5 = sum(species_mass_flow_array_bottom5,dims=1)
	for level = 1:number_of_levels5

		# get the total for this level -
		T_level_top5 = T_top5[level]
		T_level_bottom5 = T_bottom5[level]

		# grab -
		total_mdot_top_array5[level] = T_level_top5
		total_mdot_bottom_array5[level] = T_level_bottom5

		for species_index = 1:ℳ
			species_mass_fraction_array_top5[species_index,level] = (1/T_level_top5)*
				(species_mass_flow_array_top5[species_index,level])
			species_mass_fraction_array_bottom5[species_index,level] = (1/T_level_bottom5)*
				(species_mass_flow_array_bottom5[species_index,level])
		end
	end
end

# ╔═╡ ef42a945-7a58-4fc2-a3b9-605069623689
begin

	stages5 = (1:number_of_levels5) |> collect
	plot(stages5,species_mass_fraction_array_top5[idx_target_compound5,:], linetype=:steppre,lw=2,legend=:bottomright, 
		label="Mass fraction i = isoprene Tops")
	xlabel!("Stage index l",fontsize=18)
	ylabel!("Tops mass fraction ωᵢ (dimensionless)",fontsize=18)
	
	# make a 0.99 line target line5 -
	target_line5 = 0.99*ones(number_of_levels5)
	plot!(stages5, target_line5, color="red", lw=2,linestyle=:dash, label="Target 99% purity")

end

# ╔═╡ 465dc777-e391-4003-891e-007488203db0
with_terminal() do

	# initialize some space -
	state_table = Array{Any,2}(undef, number_of_levels5, 3)
	for level_index = 1:number_of_levels5
		state_table[level_index,1] = level_index
		state_table[level_index,2] = species_mass_fraction_array_top5[idx_target_compound5, level_index]
		state_table[level_index,3] = total_mdot_top_array5[level_index]
	end
...
# ╔═╡ 51fda1f8-440c-419a-9b11-542a5e534feb
#space

# ╔═╡ 41c1214f-613d-4ca7-ae7e-9d213891e6f7
md"""
##### Financial Analysis using NPV
"""

# ╔═╡ 889d583c-8f29-4236-a8b7-34f46350ee73
md"""
###### i. Comparison with Investment Yielding 1% per Year
"""

# ╔═╡ 969f1f14-ab9b-4585-9dd4-1b05872843e6
# lifetime = 3 future years (period of time to payback the investment)
T = 3


# ╔═╡ 0236638d-e464-4bb2-b3bb-9588ee8dd274
# discount rate: risk-free interest rate (alternative investment: take $34,100 and invest at the risk-free rate)
# assume constant over the payback lifetime -
discount_rate = 0.01

# ╔═╡ 964150ad-345b-4ecb-a698-67608577e855
# Setup the cash flow array -
# we pay in year 1: $34100 (out, -ve)
# we make in year(s) 2,3 and 4: $18747.31743 (in,+ve)
CF_array = [-34100, 18747.31743, 18747.31743, 18747.31743]

# ╔═╡ 8768b830-4390-478a-8808-cbb3f6e51499
...
# ╔═╡ f68b54f9-526b-4d53-ac12-712af84f2996
# what is the PV?
PV = CF_array.*(1.0./discount_array)

# ╔═╡ 9fc295de-57a3-48ee-90d0-b5067e10a9a2
# what is the NPV?
NPV = sum(PV)

# ╔═╡ 8ce7b105-d70c-4b61-866a-1e53e2146d84
md"""
###### ii. Comparison with Investment Yielding 10% per Year
"""

# ╔═╡ c841014d-64da-40c9-82a5-eb24450d679e
# lifetime = 3 future years (period of time to payback the investment)
T2 = 3

# ╔═╡ 4adfd3c5-c165-4afa-97d2-f3623464ff35
# discount rate2: risk-free interest rate (alternative investment: take $34,100 and invest at the risk-free rate)
# assume constant over the payback lifetime -
discount_rate2 = 0.1

# ╔═╡ f37ded02-9ca7-494d-87c8-d943c59476aa
# Setup the cash flow array -
# we pay in year 1: $34100 (out, -ve)
# we make in year(s) 2,3 and 4: $18747.31743 (in,+ve)
CF_array2 = [-34100, 18747.31743, 18747.31743, 18747.31743]

# ╔═╡ f1f2b03e-0955-466b-a806-9461274b5347
# compute the discount terms -
begin
	
	# main -
	discount_array2 = ones(T2+1)
	for time_index = 2:(T2+1)
    	tmp_term = (1+discount_rate2)^(time_index-1)
    	discount_array2[time_index] = tmp_term
	end
...
# what is the PV?
PV2 = CF_array2.*(1.0./discount_array2)
...
md"""
By employing the method of producing propane-1,3-diol from sucrose, our team has successfully designed a process that is profitable compared to both alternative investments of 1% per year and 10% per year.

W​​ith the discount rate of 0.01 and 0.1, the discounted present value of future cash flows for the project will be positive after 2 and 3 years, respectively. 

Given the initial capital investment of _$_34,100 and a yearly income of _$_18,747, the project is expected to gross an equivalent of _$_21,036 in today’s money over three years with a discount rate of 0.01. Similarly, with a discount rate of 0.1, the project is expected to gross an equivalent of _$_12,522 in today’s money over three years.
...
- https://developer.mozilla.org/en-US/docs/Learn/HTML/Multimedia_and_embedding/Images_in_HTML (code for imbedding image into Pluto Notebook)

- https://www.iqsdirectory.com/articles/wastewater-evaporator.html (wastewater disposal alternative)

- Pluto Notebooks provided by the ENGRI 1120 teaching team (Example-Chip-FluxBalanceAnalysis, Example-Project-Series-Template, Example-NPV)
"""
